...
- 批量转换 VLESS、VMess、Hysteria2/Hy2、Trojan/Trojan-Go、Shadowsocks/SS 和 SOCKS5 节点
- 可按“名称前缀 + 起始数”批量重命名本次转换的节点
- SOCKS5 支持 `IP:端口:用户名:密码`、`socks5://用户名:密码@IP:端口`、`用户名:密码@IP:端口`、`socks://Base64(用户名:密码)@IP:端口#节点名`
- Shadowsocks 支持 SIP002 `ss://Base64(加密方式:密码)@服务器:端口#节点名` 以及传统整段 Base64 链接
...
- 口令绑定版可为新增节点自动创建固定槽位与设备规则
- 口令绑定版支持在节点链接末尾追加 `---口令1 口令2 口令3`，一次为同一节点导入多个自定义口令槽位
...
- 删除规则前二次确认
- 删除规则后自动复用释放的最小内网 IP
- 口令绑定版转换节点时默认为每个新增节点自动排队创建一个口令槽位，并与节点、规则一起预览和应用
- 每条设备规则都会自动对应一个口令槽位；已有规则缺少槽位时，进入口令绑定页面会自动补齐
- 口令绑定页面支持选择当前节点并重复创建整组槽位，也保留按单节点和指定数量手动创建
- 每个槽位预先绑定固定 IP 与代理节点，并依次获得 `001`、`002`、`003`……数字口令
- 未绑定设备连接 LAN/Wi-Fi 后，系统通过 DHCP、邻居表和防火墙自动引导到本地口令页
- 手机刷机或更换后输入原槽位口令，会用新 MAC 直接替换旧 MAC 并继续使用原固定 IP 和代理规则，无需后台先解绑
- 槽位绑定时会精准释放该手机的旧 DHCP 动态租约并重启 DHCP 服务，避免重连后仍获取旧 IP
- 口令换绑会强制回收槽位 IP 上的旧租约和新手机原来的动态租约，并在路由器自带 Wi-Fi 上自动让新设备重连
- 兼容会在 DHCP 服务重启时输出日志的 x86/OpenWrt 固件，后端响应可从服务日志中提取最终 JSON
- 口令槽位支持按口令、IP、MAC、设备名或节点搜索，并支持查看在线状态、修改代理节点、多选、全选和批量删除
- 后台可为已绑定槽位一键“解绑设备”，清空绑定关系并释放固定租约后，该手机重连 Wi-Fi 需重新输入口令才能再次绑定
...
执行下面一条命令后，输入 `1` 安装口令绑定版，输入 `2` 安装手动绑定 IP 版：
...
下面的命令直接安装口令绑定正式版。`--resolve` 可在路由器本地 DNS 暂时不可用时仍然校验证书并下载，安装后的在线更新也会记住该解析地址：
...
- `nftables`（现代 OpenWrt/firewall4 默认提供），用于仅引导未绑定设备的 HTTP 联网检测请求
